document.addEventListener("DOMContentLoaded", () => {
    let rekomendasiDatabase = [];
    const namaFiturAktif = 'rekomendasi'; 
    ...
    const ambilDataDariMySQL = async () => {
        try {
            // Memanggil API dengan parameter fitur murni rekomendasi
            const respon = await fetch("../admin/api.php?fitur=rekomendasi");
            ...
            if (hasilJson && hasilJson.status === "success" && Array.isArray(hasilJson.data)) {
                rekomendasiDatabase = hasilJson.data;
            } else {
            ...
            const rekomendasiGridHTML = document.getElementById("rekomendasi-grid-container");
            if (rekomendasiGridHTML) {
                rekomendasiGridHTML.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: red; padding: 40px 0; font-weight: bold;">Gagal memuat data dari database server cPanel.</p>`;
            }
        }
    };
    ...
    const setupWaLinks = () => {
        ...
        waButtons.forEach(btn => {
            btn.onclick = (e) => {
                ...
                const message = btn.getAttribute("data-message") || "Halo, saya ingin bertanya tentang rekomendasi ini.";
    ...
    const rekomendasiGrid = document.getElementById("rekomendasi-grid-container");
    const searchInput = document.getElementById("search-input");
    const filterKategori = document.getElementById("filter-kategori");
    const filterUrutkan = document.getElementById("filter-urutkan");

    const renderRekomendasi = (data) => {
        ...
        if (!data || data.length === 0) {
            rekomendasiGrid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: #666; padding: 40px 0;">Rekomendasi tidak ditemukan.</p>`;
            return;
        }
        ...
        const jalankanRenderGrid = (listData) => {
            ...
            listData.forEach(item => {
                ...
                let jalurGambar = (!item.image_type || item.image_type.trim() === "" || item.image_type === "file") ? "https://placehold.co/600x400?text=Gambar+Rekomendasi" : `../uploads/${item.image_type}`;

                const cardHTML = `
                    <article class="rekomendasi-card">
                        ...
                        <div class="card-content">
                            ...
                            <div class="card-actions">
                                <a href="rekomendasi_detail.html?id=${item.id}&fitur=${namaFiturAktif}" class="btn btn-sm btn-outline" style="text-decoration: none; text-align: center; display: inline-flex; align-items: center; justify-content: center;">Cek Detail</a>
                                <button class="btn btn-sm btn-primary btn-wa" data-message="${item.wa_message || ''}" data-phone="${item.no_wa || ''}">Tanya Dulu</button>
                            </div>
                        </div>
                    </article>
                `;
                rekomendasiGrid.insertAdjacentHTML("beforeend", cardHTML);
            });
            setupWaLinks();
        };
        ...
        if (data.length > 3) {
            ...
            rekomendasiGrid.parentNode.insertBefore(btnLihatSelengkapnya, rekomendasiGrid.nextSibling);
        }
    };
...
// SCROLL KE KATALOG
    ...
    if (btnScrollKatalog) {
        btnScrollKatalog.addEventListener("click", () => {
            const target = document.getElementById("katalog-rekomendasi");
            if (target) {
